...
A modular, action-based character controller framework for Godot 4, written in C#. It is designed around composable movement logic, deterministic update stages, and surface-aware movement handling.

Instead of a single monolithic controller, movement is built from independent *action*, *action properties*, *raycaster* and *condition* nodes, allowing complex character behavior to emerge from small, reusable components.

## Core Concepts

### Pawn

The `Pawn` class is the root node of the system and represents a controllable character. It:
...
- Performs surface detection (slopes, walls, sloped ceilings, etc.).
...
- `Condition`: A pawn component that can be used to check if some condition holds true. They can be used to automatically enable/disable other components.
